...
void teir_interpreter::iterate(std::string const& node, std::vector<teir_axis const*> axis_path, std::vector<uint64_t> index_path) {
    ...
    if ((iter_node = operation.schedule.resolve_iter_id(node)) != nullptr) {
        teir_axis const* axis = resolve_axis_id(operation, iter_node->axis);
...
void teir_interpreter::lower(teir_inv_node const* inv_node, std::vector<teir_axis const*> axis_path, std::vector<uint64_t> index_path) {
    teir_primitive const* primitive = resolve_primitive_id(operation, inv_node->primitive);
    ...
    for (std::string const& tensor_id : primitive->tensors) {
        uint64_t tensor_idx = resolve_tensor_id_idx(operation, tensor_id);
    ...
    if (primitive->ptype == teir_ptype_t::ptype_zero) {
        if (primitive->axes.at("M").size() == 1 && primitive->axes.at("N").size() == 1) {
            teir_axis const* axis_m = resolve_axis_id(operation, primitive->axes.at("M")[0]);
            teir_axis const* axis_n = resolve_axis_id(operation, primitive->axes.at("N")[0]);
...
teir_axis const* teir_interpreter::resolve_axis_id(teir_operation const& operation, std::string const& id) const {
...
teir_primitive const* teir_interpreter::resolve_primitive_id(teir_operation const& operation, std::string const& id) const {
...
teir_iter_node const* teir_interpreter::resolve_iter_node_id(teir_operation const& operation, std::string const& id) const {
...
teir_inv_node const* teir_interpreter::resolve_inv_node_id(teir_operation const& operation, std::string const& id) const {
