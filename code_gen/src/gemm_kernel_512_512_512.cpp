...
uint32_t ldr_za(int wv, int imm, int xn) {
    uint32_t rv = wv - 12; // Wv muss W12-W15 sein
    uint32_t inst = 0xE1008000;
    inst |= (rv << 13);
    inst |= ((xn & 0x1f) << 5);
    inst |= (imm & 0xf);
...
uint32_t str_za(int wv, int imm, int xn) {
    uint32_t rv = wv - 12;
    uint32_t inst = 0xE1208000;
    inst |= (rv << 13);
    inst |= ((xn & 0x1f) << 5);
    inst |= (imm & 0xf);
...
void const * generate_gemm_kernel_512_512_512() {
    ...
    kernel.add_instr(gen.ssve_ptrue(pr_t::p0, sve_size_t::s, InstGen::pr_pattern_t::pow2, 0)); // pow2 ?
    ...
    fmopa(0, 0, 0, 2, 0); // fmopa za0.s, p0/m, p0/m, z2.s, z0.s

    // tile 1
    kernel.add_instr(ldr_z(1, 7, 1)); // ldr z1, [x7, #1, mul vl]
    fmopa(1, 0, 0, 2, 1); // fmopa za1.s, p0/m, p0/m, z2.s, z1.s

    // tile 2
    kernel.add_instr(ldr_z(3, 8, 1)); // ldr z3, [x8, #1, mul vl]
    fmopa(2, 0, 0, 3, 0); // fmopa za2.s, p0/m, p0/m, z3.s, z0.s
    
    // tile 3
    fmopa(3, 0, 0, 3, 1); // fmopa za3.s, p0/m, p0/m, z3.s, z1.s
    ...
    kernel.add_branch(gen.base_b_cond("N_loop", InstGen::br_cond_t(1))); // AArch64 Bedingung: NE = 1; // b.ne N_loop
...
//  l_kernel.write( "gemm_512_512_512.bin" );
    kernel.set_kernel();
