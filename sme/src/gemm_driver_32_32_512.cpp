...
void gemm_32_32_512_reference(float const* a, float const* b, float* c, int64_t ld_a, int64_t ld_b, int64_t ld_c) {
    int const K = 512; // Da es gemm_32_32_512 ist, ist K=512
...
int main() {
    ...
    // C++ Referenz aufrufen
    gemm_32_32_512_reference(a.data(), b.data(), c_ref.data(), ld_a, ld_b, ld_c);
